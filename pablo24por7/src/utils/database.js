  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      if (databaseConfig && databaseConfig.client === 'pg') {
        // const schema = k.withSchema(databaseConfig.connection.schema)
        resolve(k)
  ...
  return new Promise((resolve, reject) => {
    builder(conn).then(builder => {
      builder.raw(query, params)
        .then(rows => {
          const responseRows = rows.rows ? rows.rows : rows
          const resp = responseRows && responseRows.length ? responseRows[0] : null
          resolve(resp)
        }).catch(error => {
          loggerUtils.error(error.stack)
          reject(error)
